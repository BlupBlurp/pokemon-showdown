...
export class TeamValidator {
	...
	checkSpecies(set: PokemonSet, species: Species, tierSpecies: Species, setHas: { [k: string]: true }) {
		...
		if (isGigantamaxForm) {
			banReason = ruleTable.check("tag:gigantamax", setHas);
			if (banReason) {
				return `${tierSpecies.name} is ${banReason}.`;
			}
		}
		...
		if (isTeraForm) {
			banReason = ruleTable.check("tag:teraforme", setHas);
			if (banReason) {
				return `${tierSpecies.name} is ${banReason}.`;
			}
		}
		...
		if (isStellarForm) {
			banReason = ruleTable.check("tag:stellarforme", setHas);
			if (banReason) {
				return `${tierSpecies.name} is ${banReason}.`;
			}
		}
		...
		if (isPikachuCapForm) {
			banReason = ruleTable.check("tag:pikachucap", setHas);
			if (banReason) {
				return `${tierSpecies.name} is ${banReason}.`;
			}
		}
	...
	allSources(species?: Species) {
		...
		const maxSourceGen = this.ruleTable.has("allowtradeback") ? Utils.clampIntRange(effectiveDexGen + 1, 1, effectiveDexGen) : effectiveDexGen;
	...
	checkCanLearn(
		...
		const dex = this.dex;
		const effectiveDexGen = this.dex.currentMod === "gen8relumi" ? 9 : dex.gen;
		...
		for (const { species, learnset } of fullLearnset) {
			...
			for (let learned of sources) {
				...
				if (learnedGen >= 8 && learned.charAt(1) === 'E' && learned.slice(1) !== 'Eany' &&
					...
					if (learnedGen === effectiveDexGen && learned.charAt(1) !== 'R') {
