...
export class TeamValidator {
	...
	constructor(format: string | Format, dex = Dex) {
		...
		if (this.format.effectType !== "Format") {
			throw new Error(
				`format '${format}' should be a 'Format', but was a '${this.format.effectType}'`,
			);
		}
	...
	getValidationSpecies(set: PokemonSet): {
		...
		if (
			ability.id === "owntempo" &&
			toID(species.baseSpecies) === "rockruff"
		) {
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (
			...
			if (!type.exists || type.isNonstandard) {
				...
				species.requiredTeraType !== type.name &&
				ruleTable.has("obtainablemisc")
			) {
				problems.push(
					`${species.name}'s Terastal type needs to be ${species.requiredTeraType}.`,
				);
			}
		...
		if (
			...
			for (const source of setSources.sources) {
				if (isUnderleveled) {
					if (source.charAt(1) === "S") {
						const eventSpecies = dex.species.get(
							source.substr(3),
						).baseSpecies;
						const underleveledSpecies =
							dex.species.get(isUnderleveled).baseSpecies;
						// Can only be an FE species or in a three-stage line, the prevo
						if (
							eventSpecies !== species.baseSpecies &&
							eventSpecies !== underleveledSpecies
						)
							continue;
					} else if (source !== "8V") {
						continue;
			...
			if (legalSources.length) {
				...
				const firstEventSource = setSources.sources.find(
			...
			if (
				ruleTable.has("fullarceusclause") &&
				eventSpecies.baseSpecies === "Arceus"
			) {
				...
				eventData = [
					...eventData,
					{
						generation: 4,
						level: 80,
						moves: ["refresh", "futuresight", "recover", "hyperbeam"],
					},
				];
			}
			let legal = false;
	...
	fatherCanLearn(
		...
		const canBreedWithSmeargle =
			species.eggGroups.includes("Field") &&
			!this.dex.species.get("Smeargle").isNonstandard;
	...
	validateForme(set: PokemonSet) {
		...
		if (species.name === "Necrozma-Ultra") {
			...
			if (
				species.name === "Zygarde-Complete" ||
				species.name === "Zygarde-Mega"
			) {
				problems.push(
					`${species.name} must start the battle as Zygarde or Zygarde-10% with Power Construct. Please specify which Zygarde it should start as.`,
				);
			}
			if (item.id === "zygardite" && set.ability !== "Power Construct") {
				problems.push(
					`Zygarde holding Zygardite can only Mega Evolve with the Power Construct ability.`,
				);
			}
	...
	checkSpecies(
		...
		const isGigantamaxForm =
			...
			tierSpecies.id.endsWith("gmax") ||
			species.id.includes("gigantamax") ||
			tierSpecies.id.includes("gigantamax") ||
			species.forme === "Gigantamax" ||
			tierSpecies.forme === "Gigantamax" ||
		...
		if (isGmax) {
			...
			if (banReason) {
				return `Gigantamax Pokemon are ${banReason}.`;
			}
	...
	allSources(species?: Species) {
		const effectiveDexGen =
			this.dex.currentMod === "gen8relumi" ? 9 : this.dex.gen;
		...
		const maxSourceGen = this.ruleTable.has("allowtradeback")
			? Utils.clampIntRange(effectiveDexGen + 1, 1, effectiveDexGen)
			: effectiveDexGen;
	...
	validatePokemonGo(
		...
		if (!pokemonGoData) {
			...
			const otherSpecies = this.dex.species.get(
				species.changesFrom || species.prevo,
			);
	...
	checkCanLearn(
		...
		const dex = this.dex;
		const effectiveDexGen =
			this.dex.currentMod === "gen8relumi" ? 9 : dex.gen;
		if (!setSources.size())
		...
		let babyOnly = "";
		let minLearnGen = effectiveDexGen;
		...
		for (const { species, learnset } of fullLearnset) {
			...
			const checkingPrevo =
				species.baseSpecies !== originalSpecies.baseSpecies;
			...
			const formeCantInherit = dex.species.eggMovesOnly(
				species,
				baseSpecies,
			);
			...
			if (moveid === "sketch") {
				...
				if (move.flags["nosketch"] || move.isZ || move.isMax) {
					...
					(dex.gen === 8 ||
						(dex.gen === 9 &&
							["gen9dlc1", "gen9predlc"].includes(format.mod)))
				) {
			...
			for (let learned of sources) {
				...
				if (
					formeCantInherit &&
					(learned.charAt(1) !== "E" || learnedGen < 9)
				)
					continue;
				if (
					setSources.learnsetDomain &&
					!setSources.learnsetDomain.includes(
						`${learnedGen}${toID(species.baseSpecies)}`,
					) &&
				...
				if (noFutureGen && learnedGen > effectiveDexGen) {
					if (!cantLearnReason) {
						cantLearnReason = `can't be transferred from Gen ${learnedGen} to ${effectiveDexGen}.`;
					}
					continue;
				}
				...
				if (learned.charAt(1) === "L") {
					...
					if (level >= parseInt(learned.substr(2)) || learnedGen === 7) {
						...
						this.ruleTable.has("pomegglitchclause") &&
						level >= 5 &&
						learnedGen === 3 &&
						species.canHatch
					) {
				...
				if (
					...
					learned =
						`${learnedGen}E${species.prevo ? species.id : ""}` as MoveSource;
			...
			if (canUseHomeRelearner) {
				const fullSources = [];
				let learnsetData = this.getExternalLearnsetData(
					species.id,
					"gen8bdsp",
				);
				if (
					!["nincada", "spinda"].includes(species.id) &&
					learnsetData?.learnset?.[move.id]
				) {
					fullSources.push(...learnsetData.learnset[move.id]);
				}
				learnsetData = this.getExternalLearnsetData(
					species.id,
					"gen8legends",
				);
				...
				for (const source of fullSources) {
					...
					if (
						source.charAt(1) === "L" &&
						level < parseInt(source.substr(2))
					)
						continue;
					return null;
				}
		...
		if (!tradebackEligible) {
			...
			while (nextSpecies.name) {
				for (let gen = nextSpecies.gen; gen <= effectiveDexGen; gen++) {
					/**
