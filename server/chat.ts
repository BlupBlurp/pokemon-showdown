...
export const Chat = new class {
	...
	getDataPokemonHTML(species: Species, options: DataHTMLRenderOptions = {}) {
		...
		buf += `<span class="col pokemonnamecol" style="white-space:nowrap"><a href="${Chat.getLuminescentPokemonUrl(species)}" target="_blank">${species.name}</a>${text.name}</span> `;
	...
	getDataMoveHTML(move: Move, options: DataHTMLRenderOptions = {}) {
		...
		buf += `<span class="col movenamecol"><a href="${Chat.getLuminescentMoveUrl(move.id)}">${text.name}</a></span> `;
	...
	getDataAbilityHTML(ability: Ability, options: DataHTMLRenderOptions = {}) {
		...
		let buf = `<ul class="utilichart"><li class="result">`;
		buf += `<span class="col namecol"><a href="${Chat.getLuminescentAbilityUrl(ability.id)}">${text.name}</a></span> `;
		if (!options.hideShortDescription) {
	...
	getDataItemHTML(item: Item, options: DataHTMLRenderOptions = {}) {
		...
		let buf = `<ul class="utilichart"><li class="result">`;
		buf += `<span class="col itemiconcol"><psicon item="${item.id}"></span> <span class="col namecol"><a href="${Chat.getLuminescentItemUrl(item.id)}">${text.name}</a></span> `;
		if (!options.hideShortDescription) {
