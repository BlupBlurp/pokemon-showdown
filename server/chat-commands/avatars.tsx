...
if (
	(Config.customavatars && Object.keys(Config.customavatars).length) ||
	Config.allowedavatars
) {
	Monitor.crashlog(
...
export const Avatars = new (class {
	...
	async validate(avatar: string, options?: { rejectOfficial?: boolean }) {
		...
		if (!(await this.exists(avatar))) {
			throw new Chat.ErrorMessage(
				`Avatar "${avatar}" doesn't exist. ${AVATAR_FORMATS_MESSAGE}`,
			);
		}
				...
				`|pm|~|${user.getIdentity()}|/raw ` +
					Chat.html`${(
						<>
							...
							<p>
								{entry.allowed.map(
									(avatar) => avatar && [Avatars.img(avatar), " "],
								)}
							</p>
							Use{" "}
...
const OFFICIAL_AVATARS_WISTERIAPURPLE = new Set(["miku-fairy"]);

const OFFICIAL_AVATARS_FLAMIBANE = new Set(["miku-ghost"]);

const OFFICIAL_AVATARS_RADU = new Set(["miku-ice"]);
...
export const commands: Chat.ChatCommands = {
	avatar(target, room, user) {
		...
		if (!avatar) {
			...
			throw new Chat.ErrorMessage(
				"Unrecognized avatar - make sure you're on the right account?",
			);
		}
		...
		if (!silent || this.broadcasting) {
			...
			if (OFFICIAL_AVATARS_WISTERIAPURPLE.has(avatar)) {
				this.sendReply(
					`|raw|(${this.tr`Artist: `}<a href="https://www.smogon.com/forums/members/664887/">wisteriapurple</a>)`,
				);
			}
			if (OFFICIAL_AVATARS_FLAMIBANE.has(avatar)) {
				this.sendReply(
					`|raw|(${this.tr`Artist: `}<a href="https://www.smogon.com/forums/members/706228/">Flamibane</a>)`,
				);
			}
			if (OFFICIAL_AVATARS_RADU.has(avatar)) {
				this.sendReply(
					`|raw|(${this.tr`Artist: `}<a href="https://www.smogon.com/forums/members/455774/">RADU</a>)`,
				);
			}
		}
	},
	...
	avatars(target, room, user) {
		...
		const targetUser =
			this.broadcasting && !target ? null : this.getUserOrSelf(target);
		...
		if (targetUserids && (targetUser === user || user.can("alts"))) {
			...
			if (!out.length && target) {
				out.push(
					<p>
						User <strong>{toID(target)}</strong> doesn't have any custom
						avatars.
					</p>,
				);
			}
		...
		if (!out.length) {
			out.push(
				<p>
					Custom avatars require you to be a contributor/staff or win a
					tournament prize.
				</p>,
			);
		}
		...
		this.sendReplyBox(
			<>
				{!target && [
					...
					<p>
						Avatars from generations other than 4-5 are hidden. You can
						find them in this {}
	...
	async defaultavatar(target, room, user) {
		...
		if (!Users.isUsername(inputUsername)) {
			throw new Chat.ErrorMessage(
				`"${inputUsername}" is not a valid username.`,
			);
		}
		...
		if (!Avatars.addPersonal(userid, avatar)) {
			throw new Chat.ErrorMessage(
				`User "${inputUsername}" can already use avatar "${avatar}".`,
			);
		}
	...
	removeavatar(target, room, user) {
		...
		if (!allowed) {
			throw new Chat.ErrorMessage(
				`${inputUsername} doesn't have any custom avatars.`,
			);
		}
		if (avatar) {
			if (!Avatars.removeAllowed(userid, avatar)) {
				throw new Chat.ErrorMessage(
					`${inputUsername} doesn't have access to avatar "${avatar}"`,
				);
			}
			...
			this.sendReplyBox(
				<div>
					...
					<br />
					Removed from{" "}
					<username class="username">{inputUsername}</username>
				</div>,
			);
		} else {
			// delete all
	...
	async avatarusers(target, room, user) {
		...
		if (!Avatars.userCanUse(user, target) && !user.can("alts")) {
			throw new Chat.ErrorMessage(
				`You don't have access to avatar "${target}"`,
			);
		}
			...
			<>
				...
				<p>
					...
					{users ? (
						...
						<p>No users currently allowed to use this avatar</p>
					)}
				</p>
			</>,
		);
	...
	async masspavatar(target, room, user) {
		...
		const usernames = target
			...
			.split(/\s*\n|,\s*/)
			.map((username) =>
				username.endsWith(".png") ? username.slice(0, -4) : username,
			);
	...
	async massxmasavatar(target, room, user) {
		...
		const usernames = target
			...
			.map((username) =>
				username.endsWith("xmas") ? username.slice(0, -4) : username,
			);
		for (const username of usernames) {
			if (!Users.isUsername(username)) {
				throw new Chat.ErrorMessage(`Invalid username "${username}"`);
